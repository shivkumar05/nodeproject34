  ...
  try {
    ...
    for (let i = 0; i < uniqueDates.length; i++) {
      ...
      let categoryIds = Array.from(
        new Set(routinesOnDate.map((r) => r.category))
      ).sort();

      for (let j = 0; j < categoryIds.length; j++) {
        let categoryId = categoryIds[j];
        let routinesInCategory = routinesOnDate.filter(
...
const createPlayerRoutine = async function (req, res) {
  try {
    ...
    let { userId, drills, date, time, category } = data;
    data.userId = userid;
    ...
    let playerRoutine = await routineModel.create(data);
...
const updateComplete = async function (req, res) {
  try {
    ...
    let objectsToUpdate = await routineModel.find({ userId, category });
